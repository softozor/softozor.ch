...
import BUBBLE_IMG from '../assets/banner/goodbubble.png';
...
var banner = {
  refreshSize: function() {
    banner.widthPX = parseInt(
      window.getComputedStyle(banner.html, null).getPropertyValue('width')
    );
    banner.heightPX = parseInt(
      window.getComputedStyle(banner.html, null).getPropertyValue('height')
    );
  }
};
...
function bandProto(bandIndex) {
  ...
  if (d === Infinity || d === 0) {
    ...
    this.heightW =
      (worldBandRatioToBanner - 1 + d) * 100 / worldBandRatioToBanner;
  }
...
// one image in a band
function imgProto(bandIndex, rank) {
  // the leftmost image has rank 0
  this.bandIndex = bandIndex;
  this.rank = rank;
  this.html;
  ...
  this.update = function() {
    this.html.style.left =
      band[this.bandIndex].position.xObsPX() +
      this.rank * band[this.bandIndex].widthPX +
      'px';
...
var softozor = {
  ...
  initialize: function() {
    ...
    this.html.style.height =
      banner.heightPX * worldBandRatioToBanner * softozorData.heightW / 100 +
      'px';
    ...
    this.hitbox = new hitboxProto(
      this.position,
      softozorData.heightW * 0.6,
      softozorData.heightW * 0.35,
  ...
  fall: function() {
    ...
    this.ySpeed = Math.min(
      Math.max(this.ySpeed, -0.8 * speedFactor),
      3 * speedFactor
    );
...
function positionProto(xW, yW, bandIndex) {
  ...
  this.yObsW = function() {
    ...
    if (distanceFactor === Infinity || distanceFactor === 0) return yW;
      ...
      return (
        (this.yW - (1 - 1 / worldBandRatioToBanner) * softozor.yW) /
        distanceFactor
      );
  };
...
function obstacleProto(position, sprite, hitbox, obstacleIndex) {
  ...
  this.checkOut = function() {
    if (
      this.position.xW + this.sprite.widthW <= softozor.xW &&
...
function spriteProto(position, src, alt, widthW, heightW, bandIndex) {
  ...
  this.html.style.width =
    widthW /
      bandData[this.bandIndex].distanceFactor *
      band[this.bandIndex].heightPX /
      100 +
    'px';
  this.html.style.height =
    heightW /
...
function fillImage(imgNumber) {
  for (var fillIndex = 0; fillIndex < bubbleDensity; fillIndex++) {
    var x =
      (Math.random() + imgNumber) * band[worldBandIndex].widthW +
      softozor.xW -
      softozor.xW % band[worldBandIndex].widthW;
